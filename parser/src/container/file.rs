...
use shared::io::{ReadExt, WriteExt, WriteSeekExt};

use crate::{guid::{BlobGuid, DescriptorGuid}, reflection::TypeCollection, Hash32};
...
impl KFCFile {
    ...
    pub fn from_path(path: &Path) -> Result<Self, KFCReadError> {
    ...
    pub fn get_descriptor_guids_by_type_hash(&self, type_hash: Hash32) -> Vec<DescriptorGuid> {
        self.groups.get(&type_hash).map(|info| {
